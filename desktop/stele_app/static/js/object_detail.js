/* Scheda oggetto — editing arricchito + cronologia visuale + termini in-place.
 * Riusa ChronologyBar; ha propria save perché i campi sono diversi. */
(function(){
  'use strict';
  const $=s=>document.querySelector(s);
  const $$=s=>Array.from(document.querySelectorAll(s));
  const OWNER_KIND='object';
  ...
  $('#btnSave').addEventListener('click',async()=>{
    const body={
      inventory_number:$('#fInventoryNumber').value.trim() || null,
      label:$('#fLabel').value.trim() || null,
      description:$('#fDescription').value,
      condition_note:$('#fConditionNote').value,
      decoration_present: $('#fDecorationPresent').checked ? 1 : 0,
      decoration_note:$('#fDecorationNote').value,
      restored: $('#fRestored').checked ? 1 : 0,
  ...
  // aggiunta termine (picker cerca-o-crea su object_term)
  ...
  function openTermPicker(host){
    ...
    async function search(){
      ...
      const list=await api('GET','/api/vocab/object_term?q='+encodeURIComponent(val));
      let html='';
      if(val){
      ...
      if(createEl) createEl.addEventListener('click',async()=>{
        try{
          const created=await api('POST','/api/vocab/object_term',{preferred_label:val});
    ...
    async function assign(termId){
      try{await api('POST',`/api/object/${OWNER_ID}/terms`,{term_id:termId}); location.reload();}
      catch(e){toast(e.message,'err');}
    }
  ...
  // cronologia visuale (usa il modulo condiviso)
  ...
  async function openDatingForm(host){
    ...
    form.innerHTML=`
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px">
        ...
        <label style="font-size:11px;color:var(--ink-soft)">Da (anno)
          <input id="dfFrom" type="number" style="width:100%;font:inherit;padding:5px 8px;border:1px solid var(--line-strong);border-radius:5px"></label>
    ...
    $('#dfSave').addEventListener('click',async()=>{
      ...
      try{await api('POST',`/api/object/${OWNER_ID}/datings`,body); location.reload();}
      catch(e){toast(e.message,'err');}
    });
  }
})();
